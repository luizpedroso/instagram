...
export class AllPostsComponent implements OnInit, OnDestroy {
  ...
  onFavoritesClicked(imageData){
    this.myFire.handleFavoriteClicked(imageData)
      .then(data => {
        this.notificationService.display('success', 'Image added to favorites');
      })
      .catch(error => {
        this.notificationService.display('error', 'Error adding image to favorites');
      })
  }
  ...
  onFollowClicked(imageData){
    this.myFire.followUser(imageData.uploadedBy)
      .then(data => {
        this.notificationService.display('success', 'Follow' + imageData.uploadedBy.name + "!!!");
      })
      .catch(error => {
        this.notificationService.display('error', error);
      })
  }
